...
Detection goal
Detect suspicious rundll32.exe executions that are used as a LOLBin to open URLs / FileProtocolHandler, rather than to run local DLLs.

This aligns with MITRE ATT&CK technique:
...
ParentImage = C:\Program Files (x86)\Internet Explorer\iexplore.exe
...
Filter by executable (Image),
Inspect CommandLine,
Use string matching to spot suspicious usage patterns.
How to translate a single EVTX‑ATT&CK sample into a reusable detection that could be applied to real enterprise data.
